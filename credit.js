...
const redirectURL = 'https://jonaskjeldmand.vercel.app/';  // Replace with your preferred URL
...
class Particle {
    ...
    draw() {
        creditCtx.fillStyle = `rgba(255, 255, 255, ${this.alpha})`;
        creditCtx.beginPath();
        creditCtx.arc(this.x, this.y, this.size, 0, Math.PI * 2);
        creditCtx.closePath();
        creditCtx.fill();
    }
...
function initCredit() {
    particles = [];
    creditCanvas.width = window.innerWidth;
    creditCanvas.height = window.innerHeight;

    const fontSize = Math.min(window.innerWidth / 20, 28);
    creditCtx.font = `550 ${fontSize}px "Poppins", Arial, sans-serif`;
    creditCtx.fillStyle = 'white';
    
    const text = 'Jonas Kjeldmand Jensen';
    const metrics = creditCtx.measureText(text);
    const textWidth = metrics.width;
    
    const x = creditCanvas.width - textWidth - 20;
    const y = creditCanvas.height - 20;
    
    creditCtx.fillText(text, x, y);
    
    const data = creditCtx.getImageData(0, 0, creditCanvas.width, creditCanvas.height);
...
function handleInteraction(event) {
    const rect = creditCanvas.getBoundingClientRect();
    const scaleX = creditCanvas.width / rect.width;
    const scaleY = creditCanvas.height / rect.height;
...
creditCanvas.addEventListener('click', handleClick);
creditCanvas.addEventListener('touchend', handleTouch);
...
function getTextBounds() {
    ...
    const fontSize = Math.min(window.innerWidth / 20, 28);
    return {
        x: creditCanvas.width - metrics.width - 20,
        y: creditCanvas.height - 20,
        width: metrics.width,
...
function animateCredit() {
    creditCtx.clearRect(0, 0, creditCanvas.width, creditCanvas.height);
    const currentTime = Date.now();
    for (let i = 0; i < particles.length; i++) {
        particles[i].update(mouse.x, mouse.y, currentTime);
        particles[i].draw();
    }
    requestAnimationFrame(animateCredit);
}
